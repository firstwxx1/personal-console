    ...
    <SectionPanel title="VPS 状态" actionLabel="查看全部" actionHref="/servers">
      <div className="grid gap-3 p-4 lg:grid-cols-3">
        {vpsServers.map((server) => (
          <article
            key={server.id}
            className="rounded-lg border border-border bg-elevated/50 p-3 transition-colors duration-150 hover:border-input"
          >
            ...
            <dl className="mt-3 space-y-2.5">
              {[
                ...
                { label: "内存", value: server.memory },
                { label: "磁盘", value: server.disk }
              ].map((resource) => (
                <div key={resource.label}>
                  ...
                  <dd>
                    <Progress value={resource.value} className="h-1" />
                  </dd>
                </div>
              ))}
            </dl>
